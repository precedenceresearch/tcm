...
if($reptitle!="")
{
    $condition.=" and (`predr_reports`.`reportSubject` like '%".$reptitle."%' or `predr_reports`.`reportSubject` like '%".$reptitle."' or `predr_reports`.`reportSubject` like '".$reptitle."%')";
}
if($repcategory!="")
{
    $condition.=" and (`predr_reports`.`CatId`='".$repcategory."')";
}
if($daterange!="")
{  //05/12/2022 - 06/21/2022
  ...
  // $todat=date_format($todat33,"Y:m:d");
   ...
   $condition.=" and (`predr_reports`.`created_at` BETWEEN '".$newfromdate."' and '".$newplustodate."')";
}
//echo $condition;
if($page=="1")
{
...
$fields = "report_id,reportSubject,CatId,status,footerreport,created_at,slug";
$orderbyfreshleads="`report_id` DESC";
$limit ="$nextoffset,$limitcond";
$report_details = $obj_report->getReportDetails($fields, $condition, $orderbyfreshleads, $limit,'', 0);
$all_report_cnt=$obj_report->getReportDetails($fields, $condition, $orderbyfreshleads, '', 0);
...
<?php 
    ...
    foreach ($report_details as $report_detail) {
         ...
         {
            $repurl= SITEPATH.'insights/'.$report_detail['slug'];
         }
?>
...
<tr>
     ...
     <td class="f_name"> <?php if(isset($report_detail['footerreport']) && $report_detail['footerreport']==1){ ?> <span style="color:red;">*</span> <?php } ?> <a href="<?php echo $repurl;?>" target="_blank"><?php echo $reporttitle;?></a></td>
     <td><?php echo $catname; ?></td>
     <td><?php echo date("F d, Y",strtotime($report_detail['created_at']));?></td>
    
    <td class="text-end">
        <div style="display:flex;"><a class="m-1" title="Edit Report" href="add-report?report_id=<?php echo base64_encode($report_detail['report_id']);?>">
            ...
            <div class="border d-flex">
                <a class="m-1" title="Create Graph" href="create-graph.php?report_id=<?php echo base64_encode($report_detail['report_id']);?>"><img src="<?php echo SITEADMIN."images/g.png";?>" height="35" width="40"></a>
                 <a class="m-1" title="View Graph List" href="graph-list.php?report_id=<?php echo base64_encode($report_detail['report_id']);?>">
            <img src="<?php echo SITEADMIN."images/eye.png";?>" height="30" width="30"></a>
            </div>
            <!--<a class="m-1" title="Delete Report" href="delete-report.php?report_id=<?php echo base64_encode($report_detail['report_id']);?>">-->
...
<script type="text/javascript">
	$(document).ready(function() {
		$(".page-link").click(function(){
			...
			$.ajax({
				url: "<?php echo SITEADMIN; ?>manage-report-pagination.php",
				type: "GET",
				data: {
