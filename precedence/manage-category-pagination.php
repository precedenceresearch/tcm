...
$fields = "*";
$orderbyfreshleads="`catId` DESC";
$limit ="$nextoffset,$limitcond";
$category_details = $obj_category->getCategoryDetails($fields, $condition, $orderbyfreshleads, $limit,'', 0);
$all_category_cnt=$obj_category->getCategoryDetails($fields, $condition, $orderbyfreshleads, '', 0);
$total_records = count($all_category_cnt);  
$total = $total_records;
...
<tr>
     ...
     <td><?php echo $category_detail['catId'];?></td>
     <td class="f_name">
        <?php echo $category_detail['catName'];?></td>
     <td>
        <?php echo date("F d, Y",strtotime($category_detail['created_at']));?>
     </td>
    
    <td class="text-end">
        <div style="display:flex;">
            <a class="m-1" title="Edit Category" href="add-category?cat_id=<?php echo base64_encode($category_detail['catId']);?>">
                <img src="<?php echo SITEADMIN."images/writing.png";?>" height="30" width="30"></a>
                <?php $category_detail_status = $category_detail['status']; ?>
                <input type="checkbox" name="switch_published" id="switch<?php echo $category_detail['catId'];?>" onclick="action_published(<?php echo $category_detail['catId'];?>)" value="<?php echo $category_detail_status; ?>" <?php echo $category_detail_status == "Active" ? "checked" : "unchecked" ?>/>
                <label for="switch<?php echo $category_detail['catId'];?>" id="toggle_label"></label>
...
<script type="text/javascript">
	$(document).ready(function() {
		$(".page-link").click(function(){
			...
			$.ajax({
				url: "<?php echo SITEADMIN; ?>manage-category-pagination.php",
				type: "GET",
				data: {
					page : id
				},
