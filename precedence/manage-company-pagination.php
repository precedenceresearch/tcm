...
if($repid!="")
{
    $condition.=" and (`predr_topCompanies`.`report_id`='".$repid."')";
}
if($reptitle!="")
{
    $condition.=" and (`predr_topCompanies`.`title` like '%".$reptitle."%' or `predr_topCompanies`.`title` like '%".$reptitle."' or `predr_topCompanies`.`title` like '".$reptitle."%')";
}
if($repcategory!="")
{
    $condition.=" and (`predr_topCompanies`.`CatId`='".$repcategory."')";
}
if($daterange!="")
{  //05/12/2022 - 06/21/2022
  ...
  // $todat=date_format($todat33,"Y:m:d");
   ...
   $condition.=" and (`predr_topCompanies`.`created_at` BETWEEN '".$newfromdate."' and '".$newplustodate."')";
}
//echo $condition;
if($page=="1")
{
...
$report_details = $obj_report->getCompanyDetails($fields, $condition, $orderbyfreshleads, $limit,'', 0);
$all_report_cnt=$obj_report->getCompanyDetails($fields, $condition, $orderbyfreshleads, '', 0);
...
<tr>
     ...
     <td class="f_name"> <a href="<?php echo SITEPATH.'companies/'.$report_detail['slug']; ?>"> <?php echo $report_detail['title']; ?></a></td>
     <td><?php echo date("F d, Y",strtotime($report_detail['createddate']));?></td>
    
    <td class="text-end">
        <div style="display:flex;"><a class="m-1" title="Edit Companies" href="add-company?id=<?php echo base64_encode($report_detail['id']);?>">
            <img src="<?php echo SITEADMIN."images/writing.png";?>" height="30" width="30"></a>
            <?php $report_detail_status = $report_detail['status']; ?>
           <input type="checkbox" name="switch_published" id="switch<?php echo $report_detail['id'];?>" onclick="action_published(<?php echo $report_detail['id'];?>)" value="<?php echo $report_detail_status; ?>" <?php echo $report_detail_status == "Active" ? "checked" : "unchecked" ?>/>
           <label for="switch<?php echo $report_detail['id'];?>" id="toggle_label" title="Change Status"></label>
...
<script type="text/javascript">
	...
	function action_published(repid)
        ...
        // var chk_publish =  $('#switch'+campid).val();
            $.ajax({
                  url : "<?php echo SITEADMIN; ?>update-company-status-published.php",
                  type : "POST",
                  data : {repid:repid},
